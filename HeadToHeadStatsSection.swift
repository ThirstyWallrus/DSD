...
struct HeadToHeadStatsSection: View {
    ...
    // MARK: - Match history (split into smaller helpers to avoid compiler type-check complexity)

    private var matchHistoryView: some View {
        Group {
            if let snaps = matchSnapshots, !snaps.isEmpty {
                snapshotMatchesView
            } else if let list = h2hDetails, !list.isEmpty {
                legacyMatchesView
            } else {
    ...
    // Pre-compute a sorted list of snapshots (explicitly typed) to keep the View builder simple.
    private var snapshotSortedMatches: [H2HMatchSnapshot] {
        guard let snaps = matchSnapshots else { return [] }
        return snaps.sorted { (lhs: H2HMatchSnapshot, rhs: H2HMatchSnapshot) -> Bool in
    ...
    private var snapshotMatchesView: some View {
        VStack(alignment: .leading, spacing: 8) {
            ...
            VStack(spacing: 8) {
                ForEach(snapshotSortedMatches) { match in
                    matchRowView(match)
                }
            }
        }
    ...
    private var legacyMatchesView: some View {
        VStack(alignment: .leading, spacing: 8) {
            Divider().background(Color.white.opacity(0.06))
            Text("Match History")
                .font(.subheadline.bold())
            ...
            let sorted = sortedDetails(h2hDetails ?? [])
            VStack(spacing: 8) {
